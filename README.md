...
`tools/list` at `https://gateway.pipeworx.io/cms/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/cms_search_datasets \
  -H 'Content-Type: application/json' \
  -d '{"keyword":"hospital"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/cms_search_datasets`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
